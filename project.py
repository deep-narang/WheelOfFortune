...
#very important method
    def getPossibleLetters(self,guessed):
        ...
        if self.prizeMoney<VOWEL_COST:
            for vowle in VOWELS:
                impossible_list.append(vowle)

            impossible_list.extend(guessed)

        else:
            impossible_list=guessed

        for ltr in LETTERS:
            if ltr not in impossible_list:
                possible_guess.append(ltr)

        return possible_guess    

#very important method
    def getMove(self, category, obscuredPhrase, guessed):
        possible_guess_lst=self.getPossibleLetters(guessed)

        if len(possible_guess_lst)==0:
            return "pass"

        for ltr in possible_guess_lst:
            if (ltr in VOWELS) and (self.prizeMoney<VOWEL_COST):
                return "pass" 

        if self.smartCoinFlip() is True:
            for ltr in self.SORTED_FREQUENCIES[::-1]:
                if ltr in possible_guess_lst:
                    return ltr

        else:
            return random.choice(possible_guess_lst)
